...
cartItemsRoutes.post("/cart-items", (req, res) => {
  ...
  pool
    .query(
      "insert into ShoppingCart (product, id, price, quantity) values ($1::text, $2::int, $3::int, $4::int)",
...
cartItemsRoutes.put("/cart-items/:id", (req, res) => {
  ...
  pool
    .query(
      "update ShoppingCart set product=$1::text, id=$2::int, price=$3::int, quantity=$4::int",
      [req.body.product, req.body.id, req.body.price, req.body.quantity]
    )
    .then(() => {
      selectWholeCart(req, res);
